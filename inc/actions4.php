<?php
	...
	function cases_before_person() {
		if ( !is_single() || get_post_type() != 'persons' )
			return;
		if ( function_exists( 'datatable_generator' ) )
			datatable_generator( array( 'fields' => 'ID, post_title:link, prioritet, state:tax', 'group' => 'prioritet', 'meta' => 'responsible:' . get_the_ID(), 'view' => 'id:dt_person' ) );
	}

	add_action( 'roots_entry_content_before', 'cases_before_person' );